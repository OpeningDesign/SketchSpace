...
`server/src/bonsaiLayout.ts` parses Bonsai layout SVGs; `scripts/import-layout.mjs`
and `scripts/export-layout.mjs` are the round trip. The README explains the model.
Things that will bite:

- **Read layouts, never `sheets/*.svg`.** The built sheet is megabytes of base64
  PNG and is regenerated by `bpy.ops.bim.create_sheets()`; anchoring to it is
  pointless.
...
- **Never re-serialise the layout XML.** String-replace the single attribute; a
  round-trip reformat destroys the diff that justifies layouts existing.
- Position is `<g transform>` *composed with* `<image x/y>`. Both matter - Bonsai
  writes the latter, Inkscape the former.
- Layout hrefs are URL-encoded with backslashes (`..%5Cdrawings%5C...`). Normalise.
