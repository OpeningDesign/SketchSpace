...
`server/src/bonsaiLayout.ts` parses layouts, `layoutSync.ts` reconciles a page
against one, `layoutWatcher.ts` watches for Bonsai's edits, `layoutWriter.ts`
writes ours back on a debounce. The README explains the model. What will bite:
